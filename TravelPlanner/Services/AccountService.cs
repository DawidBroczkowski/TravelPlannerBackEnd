...
namespace TravelPlanner.Services
{
    public class AccountService : BaseService, IAccountService
    {
        ...
        public async Task<AuthenticationDto> LoginUserAsync(LoginUserDto dto, CancellationToken cancellationToken)
        {
            ...
            (string accessToken, Guid jti, DateTime jtiExpiry, long permissions)
                = await _authService.CreateAccessTokenAsync(user, cancellationToken);

            var refreshToken = _authService.CreateRefreshToken(out DateTime refreshTokenExpiry);

            await _userRepository
                .UpdateTokensAsync(user.Id, refreshToken, refreshTokenExpiry, jti, jtiExpiry, cancellationToken);
        ...
        public async Task<AuthenticationDto> RefreshTokenAsync(RefreshTokenDto dto, CancellationToken cancellationToken)
        {
            var userDto = await _userRepository.GetUserByRefreshTokenAsync(dto.RefreshToken!, cancellationToken);

            if (userDto is null || userDto.Id != dto.UserId)
            {
                var ex = new UnauthorizedAccessException("User not found");
                throw ex;
            }
            ...
            var oldRefreshToken = await _userRepository.GetUserRefreshTokenAsync(userDto.Id, cancellationToken);
            if (oldRefreshToken != dto.RefreshToken)
            {
            ...
            {
                await _jwtBlacklistRepository
                    .BlacklistTokenAsync(userDto.Jti.ToString()!, userDto.JtiExpiry!.Value.AddMinutes(1));
            }
            ...
            await _userRepository
                .UpdateTokensAsync(user!.Id, newRefreshToken, refreshTokenExpiry, jti, jtiExpiry, cancellationToken);
